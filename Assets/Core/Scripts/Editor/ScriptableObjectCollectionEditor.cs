    ...
    public class ScriptableObjectCollectionEditor<T> : VisualElement where T : ScriptableObject
    {
        ...
        private void InitializeInternal()
        {
            ...
            Action<VisualElement, int> bindItem = (element, i) =>
            {
                ...
                SerializedObject serializedObject = new(m_FilteredListView[i]);
        ...
        private void Create()
        {
            Type[] types = AppDomain.CurrentDomain.GetAssemblies().SelectMany(assembly => assembly.GetTypes().Where(type => typeof(T).IsAssignableFrom(type) &&
